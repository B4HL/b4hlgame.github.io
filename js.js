	...
	if (!gameover) {
	    ...
	    if (danger) {
			...
			if (health<10) {
			    ...
			    ctx.fillText(" သွေနည်းနေပါပြီ။",stage.w/2,stage.h*0.6);
			}
	    } else {
	    	dangera = 0;
	    }
	    ...
	    ctx.textAlign = "left"; 
	    ctx.textBaseline = "middle"; 
	    ctx.fillText("သွေးအား :",20,40);
 
	    ctx.fillText("အမှတ်: "+score,stage.w-200,40);
	    // ctx.fillText("Step:   "+(Date.now()-steptime),20,120);
	    if (health>30) {
	    	ctx.fillStyle='#005DC5';
		} else {
	    ...
	    ctx.font = "bold 50px arial";
	    ctx.fillText("အမှတ်: "+score,stage.w/2,stage.h/2);

	    ctx.font = "bold 30px arial";

	    ctx.fillText("START",stage.w/2,stage.h*0.65);
    	...
    	ctx.fillStyle='#f0ad28';
	    ctx.font = "bold 20px arial";
	    ctx.textAlign = "center"; 
	    ctx.textBaseline = "middle"; 
	    ctx.fillText("Developer By B4HL",stage.w/2,stage.h-20);
}
